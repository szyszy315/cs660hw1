...
public class BTreeFile implements DbFile {
	...
	protected BTreeLeafPage splitLeafPage(TransactionId tid, HashMap<PageId, Page> dirtypages, BTreeLeafPage page, Field field)
		...
		int middle = (page.getNumTuples() + 1) / 2;
		Iterator<Tuple> iterator = page.reverseIterator();
		Tuple cur = null;
		for (int i = 0; i < middle && iterator.hasNext(); i ++) {
			cur = iterator.next();
			page.deleteTuple(cur);
			rightpage.insertTuple(cur);
		}


		rightpage.setRightSiblingId(page.getRightSiblingId());
		if (page.getRightSiblingId() != null) {
			BTreeLeafPage originalright = (BTreeLeafPage) this.getPage(tid, dirtypages, page.getRightSiblingId(), Permissions.READ_WRITE);
			originalright.setLeftSiblingId(rightpage.getId());
		}


		page.setRightSiblingId(rightpage.getId());
		rightpage.setLeftSiblingId(page.getId());
		//insert middle to the parent
		Field newkey = cur.getField(this.keyField());
		BTreeInternalPage parent = (BTreeInternalPage) this.getParentWithEmptySlots(tid, dirtypages, page.getParentId(), field);
		parent.insertEntry(new BTreeEntry(newkey, page.getId(), rightpage.getId()));
	...
	protected BTreeInternalPage splitInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		Field middlefield = cur.getKey();
		page.deleteKeyAndRightChild(cur);
		cur.setLeftChild(page.getId());
		cur.setRightChild(newpage.getId());
	...
	protected void stealFromLeafPage(BTreeLeafPage page, BTreeLeafPage sibling,
		...
		int difference = (sibling.getNumTuples() - page.getNumTuples()) / 2;
	...
	protected void stealFromLeftInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int difference = (leftSibling.getNumEntries() - page.getNumEntries()) / 2;
		BTreeEntry right = page.iterator().next();
		BTreeEntry left = null;
		BTreeEntry par = parentEntry;
		for (int i = 0; i < difference; i ++ ) {
			left = it.next();
			// par => right
			par.setLeftChild(left.getRightChild());
			par.setRightChild(right.getLeftChild());
			leftSibling.deleteKeyAndRightChild(left);
			//left => parent
			parent.deleteKeyAndRightChild(par);
			page.insertEntry(par);
	...
	protected void stealFromRightInternalPage(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		int difference = (rightSibling.getNumEntries() - page.getNumEntries()) / 2;
		BTreeEntry right;
		for (int i = 0; i < difference; i++) {
			right = it.next();

			// parent to left
			parent.deleteKeyAndLeftChild(par);
			par.setLeftChild(left.getRightChild());
			par.setRightChild(right.getLeftChild());
			page.insertEntry(par);
			//insert right to partent
	...
	protected void mergeLeafPages(TransactionId tid, HashMap<PageId, Page> dirtypages,
		...
		while (it.hasNext()) {
			right = it.next();
			rightPage.deleteTuple(right);
			leftPage.insertTuple(right);
		}
